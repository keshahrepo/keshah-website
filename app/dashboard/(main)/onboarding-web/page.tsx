// WEB VARIANT of /dashboard/onboarding.
//
// Same quiz-answer breakdown + funnel stages as the mobile onboarding
// dashboard, filtered to users who came through the /start web funnel:
// `payment_provider == "stripe"` OR `signup_source == "web_onboarding"`.
//
// Adds a "Landed → Started trial" conversion card up top, sourced from
// FunnelEvents (landingHook unique sessions for source="us") against the
// count of web users who reached the trial-started state — the first
// top-of-funnel metric that's specific to web (mobile has no equivalent
// landing surface).
//
// Web users don't have the mobile-specific step markers
// (founder_story_started_at etc.) written to their Users doc because
// those are set by PostAuthFlow2 in the mobile app. The funnel drop-off
// panel therefore stays empty for web-only users until we backfill the
// same timestamps from FunnelEvents onto Users at attach-identity time.
// Quiz answer cards work today because attach-identity writes the same
// quiz-answer fields as PostAuthFlow2.
...
export default async function QuizResponsesPage({
  ...
  // WEB DASHBOARD: we filter in-memory to `payment_provider == "stripe"`
  // OR `signup_source == "web_onboarding"` so only /start-funnel users
  ...
  // Web-only filter — used to strip mobile users out of every metric on
  // this page. attach-identity writes payment_provider="stripe" for every
  // paid web trial. signup_source="web_onboarding" is a belt-and-suspenders
  // fallback in case an older web user didn't get payment_provider set.
  const isWebUser = (data: Record<string, unknown>): boolean =>
  ...
  return (
    <div>
      <header style={{ marginBottom: 20 }}>
        ...
        >
          {scopeLabel} · web signups since{" "}
          {RELEASE_CUTOFF.toISOString().slice(0, 10)} ·{" "}
