  ...
  return (
    <div>
      ...
      <div style={{
        ...
        <MetricCard label="Total Clicks" value={allClicks.toLocaleString()} />
      ...
      {showForm && (
        <form onSubmit={handleAddCreator} style={{
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 20 }}>
            ...
            <div>
              <label style={labelStyle}>Short Link Slug</label>
              <input
                value={formSlug}
                onChange={(e) => setFormSlug(e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, ""))}
                placeholder={formName.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "") || "sarah"}
                style={inputStyle}
              />
              <p style={{ fontSize: 11, color: "rgba(255,255,255,0.3)", marginTop: 4 }}>
                keshah.com/{formSlug || formName.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "") || "..."}
              </p>
            </div>
          </div>
