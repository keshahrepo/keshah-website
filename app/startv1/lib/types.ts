...
export const STEP_ORDER: StartStep[] = [
  ...
  "momentFounderFlashback",
  "trialPaywall",
  // "payment" step removed — trial paywall now reveals the inline
  // Stripe Elements checkout below itself on Continue tap (same-domain
  // card capture, no redirect). The old PaymentStep component that
  // redirected to Stripe hosted checkout is deprecated. `"payment"` is
  // still in the StartStep union above for compatibility with any old
  // persisted flow-state referencing it.
];
