...
export default function SuccessClient({
  ...
  // StartTrial fires from the bridge domain (plan-confirmed.vercel.app),
  // NOT here. Stripe's success_url now points directly at the bridge,
  // which fires browser pixel + CAPI on the fresh bridge pixel with
  // event_id = checkout session id, then bounces the user here with
  // ?bridge_done=1. Same event_id used by the webhook CAPI fire →
  // Meta dedupes.
  //
  // Fallback: if NEXT_PUBLIC_BRIDGE_URL is unset OR the user somehow
  // reached /success without going through the bridge (e.g. bookmarked
  // URL, direct paste), fire the legacy local pixel as a backup so
  // conversion tracking doesn't go dark.
  ...
  useEffect(() => {
    ...
    const bridgeDone = params.get("bridge_done") === "1";
    const bridgeUrl = process.env.NEXT_PUBLIC_BRIDGE_URL;
    startTrialFiredRef.current = true;
    if (bridgeUrl && bridgeDone) return; // bridge already fired
